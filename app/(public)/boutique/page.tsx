...
import { useBoutiqueProduits, useTemoignages } from "@/app/hooks/use-vitrine";
...
function stockLabel(produit: Produit): { label: string; tone: string } {
  ...
  if (produit.statut === "ARCHIVE") return { label: "Archivé", tone: "bg-slate-100 text-slate-600" };
...
export default function BoutiquePage() {
  ...
  const { data: produits, loading: produitsLoading } = useBoutiqueProduits();
  ...
  const visibleProduits = useMemo(() => {
    ...
    return produits.filter(
      (p) =>
        (category === "Tous" || (p.filiale?.nom ?? "Général") === category) &&
        (filiale === "Toutes" || (p.filiale?.nom ?? "WUGAMS") === filiale)
    );
  ...
  function addToCart(produit: Produit) {
    setCart((prev) => {
      ...
      if (existing) {
        return prev.map((item) => (item.produit.id === produit.id ? { ...item, quantity: item.quantity + 1 } : item));
      }
  ...
  function updateQuantity(produitId: string, delta: number) {
    setCart((prev) =>
      prev
        .map((item) => (item.produit.id === produitId ? { ...item, quantity: Math.max(1, item.quantity + delta) } : item))
  ...
  const hasAvis = temoignages && temoignages.length > 0;
    ...
    <main className="min-h-screen bg-[#fbfcfe] text-[#17294b]">
      ...
      <section className="bg-[#101a2d] pb-10 pt-32 text-white">
        <div className="mx-auto w-full max-w-[1240px] px-5 sm:px-8">
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-amber-300">Espace Wu · Livraison 7 j/7</p>
          <h1 className="mt-3 text-4xl font-bold tracking-[-0.05em] sm:text-5xl">Entretien, matériaux, mobilier &amp; outillage</h1>
          <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">
            Commandez en ligne et faites-vous livrer sur votre chantier, même en urgence. Paiement par carte ou Mobile Money.
          </p>
        ...
        {produitsLoading ? (
          ...
          <>
                  ...
                  >
                    <button className="block w-full text-left" onClick={() => setSelectedProduit(produit)} type="button">
                      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                        ...
                        <span className={"absolute right-3 top-3 rounded-full px-2.5 py-1 text-[10px] font-bold " + stock.tone}>
                          {stock.label}
                        </span>
                      </div>
                      <div className="p-4">
                        <div className="flex items-start justify-between gap-2">
                          <h3 className="text-sm font-bold text-[#233856]">{produit.nom}</h3>
                          <span className="shrink-0 text-[11px] font-semibold text-slate-400">
                            {produit.quantite_actuelle} dispo
                          </span>
                        </div>
                        <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-slate-400">
                          {produit.description ?? produit.reference} · {produit.filiale?.nom ?? ""}
                        </p>
                        <p className="mt-1 text-[11px] font-medium text-slate-400">
                          {produit.filiale?.nom ?? "WUGAMS"} · {produit.quantite_actuelle} unités
                        </p>
                      </div>
                    </button>
                    <div className="flex items-center justify-between gap-2 border-t border-slate-100 p-4 pt-3">
                      <p className="text-[15px] font-bold text-[#17294b]">{formatFcfa(Number(produit.prix_unitaire))}</p>
                      <button
                        className="inline-flex items-center gap-1.5 rounded-lg bg-[#e3a641] px-3 py-2 text-xs font-bold text-[#14223b] transition hover:bg-[#efb653] disabled:opacity-50"
            ...
            {visibleProduits.length === 0 ? (
              <div className="mt-8 grid place-items-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 py-12 text-center">
                ...
                <p className="mt-3 text-sm font-bold text-slate-600">Aucun produit dans cette catégorie</p>
                <p className="mt-1 text-xs text-slate-400">Essayez un autre filtre.</p>
              </div>
            ) : null}
            <p className="mt-8 text-center text-xs text-slate-400">Les prix affichés incluent la TVA. Livraison offerte dès 100 000 FCFA.</p>
          </>
        ) : (
          <div className="grid place-items-center rounded-2xl border border-dashed border-slate-200 bg-white py-16 text-center shadow-sm">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
              Aucun produit n&apos;est disponible pour le moment. Le Gérant prépare le catalogue depuis l&apos;espace d&apos;administration.
            </p>
            <Link
              href="/"
              className="mt-6 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-600 transition hover:border-slate-300"
            >
              Retour à l&apos;accueil
            </Link>
          </div>
      ...
      {/* Avis produits — masqué si aucun témoignage */}
      {hasAvis ? (
        <section className="border-t border-slate-200 bg-[#f7f9fc]">
          <div className="mx-auto w-full max-w-[1240px] px-5 py-12 sm:px-8">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#d19331]">Avis produits · clients vérifiés</p>
                <h2 className="mt-2 text-2xl font-bold tracking-[-0.04em] text-[#17294b]">Ce que disent nos clients</h2>
              </div>
              <p className="hidden text-sm font-bold text-slate-500 sm:block">★ {temoignages!.length} avis</p>
            </div>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {temoignages!.slice(0, 3).map((review) => (
                <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" key={review.id}>
                  <div className="flex items-center gap-1 text-amber-500">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg key={star} className={"size-3.5 " + (star <= review.rating ? "fill-current" : "fill-slate-200")} viewBox="0 0 20 20">
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                    ...
                    <p className="text-[11px] font-semibold text-slate-400">{review.role}</p>
                  </div>
                </article>
              ))}
            </div>
      ...
      {cartOpen ? (
        <div className="fixed inset-0 z-[60]">
          <button aria-label="Fermer le panier" className="absolute inset-0 bg-slate-950/45" onClick={() => setCartOpen(false)} type="button" />
          <aside className="absolute inset-y-0 right-0 flex w-full max-w-[400px] flex-col bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              ...
              <button aria-label="Fermer" className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" onClick={() => setCartOpen(false)} type="button">
            ...
            {cart.length === 0 ? (
              <div className="grid flex-1 place-items-center p-8 text-center">
                <div>
                  ...
                  <button className="mt-4 rounded-xl bg-[#17294b] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#243a61]" onClick={() => setCartOpen(false)} type="button">
                    Voir le catalogue
                  </button>
                </div>
              </div>
              ...
              <>
                  ...
                  {cart.map((item) => (
                    <div className="flex gap-3 py-4" key={item.produit.id}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <p className="truncate text-xs font-bold text-[#233856]">{item.produit.nom}</p>
                          <button aria-label={"Retirer " + item.produit.nom} className="text-slate-300 transition hover:text-[#db6d5b]" onClick={() => removeItem(item.produit.id)} type="button">
                            <Icon name="trash" size={15} />
                          </button>
                        </div>
                        <p className="mt-0.5 text-[11px] font-medium text-slate-400">{formatFcfa(Number(item.produit.prix_unitaire))}</p>
                        <div className="mt-2 flex items-center gap-2">
                          <button aria-label="Diminuer la quantité" className="grid size-6 place-items-center rounded-md border border-slate-200 text-slate-500 transition hover:border-slate-300" onClick={() => updateQuantity(item.produit.id, -1)} type="button">
                            <Icon name="minus" size={13} />
                          </button>
                          <span className="w-6 text-center text-xs font-bold text-[#233856]">{item.quantity}</span>
                          <button aria-label="Augmenter la quantité" className="grid size-6 place-items-center rounded-md border border-slate-200 text-slate-500 transition hover:border-slate-300" onClick={() => updateQuantity(item.produit.id, 1)} type="button">
                            <Icon name="plus" size={13} />
                          </button>
                        </div>
                      </div>
                      <p className="shrink-0 text-xs font-bold text-[#17294b]">{formatFcfa(Number(item.produit.prix_unitaire) * item.quantity)}</p>
                    </div>
                  ))}
                </div>
                <div className="border-t border-slate-100 px-5 py-4">
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">{cartTotal >= 100000 ? "Livraison offerte sur Abidjan." : "Livraison Abidjan : 4 500 FCFA."}</p>
                  <button className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#e3a641] px-4 py-3 text-sm font-bold text-[#14223b] shadow-lg shadow-amber-600/15 transition hover:bg-[#efb653]" onClick={() => setCheckoutOpen(true)} type="button">
        ...
        <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/45 p-4">
          <div aria-labelledby="checkout-title" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl" role="dialog">
            <div className="flex items-start justify-between gap-4">
              ...
              <button aria-label="Fermer" className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" onClick={() => setCheckoutOpen(false)} type="button">
                <Icon name="close" size={18} />
              </button>
            </div>
                ...
                {paymentMethods.map((method) => (
                  <button
                    ...
                    className={
                      ...
                      (payment === method.id ? "border-[#e3a641] bg-amber-50/70 ring-2 ring-amber-200/60" : "border-slate-200 bg-white hover:border-slate-300")
                    }
                    key={method.id}
            ...
            <div className="mt-6 flex justify-end gap-2.5">
              <button className="rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs font-bold text-slate-600 transition hover:border-slate-300" onClick={() => setCheckoutOpen(false)} type="button">
                Annuler
              </button>
              <button className="inline-flex items-center gap-2 rounded-xl bg-[#17294b] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#243a61]" onClick={submitOrder} type="button">
      ...
      {orderDone ? (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/45 p-4">
          <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-8 text-center shadow-2xl">
            ...
            <button className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-[#e3a641] px-5 py-3 text-sm font-bold text-[#14223b] shadow-lg shadow-amber-600/15 transition hover:bg-[#efb653]" onClick={() => setOrderDone(false)} type="button">
          ...
          <div aria-labelledby="product-detail-title" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-2xl" role="dialog">
            ...
            <div className="p-6">
              ...
              <div className="mt-5 flex gap-2.5">
                <button className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600 transition hover:border-slate-300" onClick={() => setSelectedProduit(null)} type="button">
                  Fermer
                </button>
                <button className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#e3a641] px-4 py-2.5 text-sm font-bold text-[#14223b] transition hover:bg-[#efb653] disabled:opacity-50" disabled={selectedProduit.statut === "RUPTURE"} onClick={() => { addToCart(selectedProduit); setSelectedProduit(null); }} type="button">
