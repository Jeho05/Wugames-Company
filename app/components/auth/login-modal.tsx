...
export function LoginModal({ open, onClose, onSuccess }: LoginModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center p-4 animate-[fadeIn_0.2s_ease-out]">
      <button
        aria-label="Fermer"
      ...
      <div
        ...
        aria-modal="true"
        className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl animate-[slideUp_0.3s_ease-out]"
        role="dialog"
      >
        <button
          aria-label="Fermer"
        ...
        <div className="mb-6">
          <BrandMark />
          <div className="mt-4 flex items-center gap-2 rounded-full bg-[#edf6ff] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.12em] text-[#426b95] w-fit">
            <Icon name="lock" size={14} />
            Connexion
          </div>
          <h2 className="mt-3 text-2xl font-bold tracking-[-0.04em] text-[#17294b]" id="login-modal-title">
        ...
        {pending2fa ? (
          <form className="space-y-4" onSubmit={handleTwoFactorSubmit}>
            ...
            <div>
              <label className="mb-1.5 block text-xs font-bold text-slate-700" htmlFor="modal-2fa-token">
                Code 2FA
              </label>
              <input
            ...
            <LoadingButton
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#17294b] px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-slate-900/15 transition hover:bg-[#243a61] disabled:cursor-not-allowed disabled:opacity-60"
              loading={submitting}
          ...
          <form className="space-y-4" onSubmit={resetView === "reset-request" ? handleResetRequest : handleResetConfirm}>
            <div className="rounded-xl border border-sky-100 bg-[#edf6ff] px-3.5 py-2.5">
              <p className="text-[11px] leading-5 text-sky-800">
            ...
            {resetView === "reset-request" ? (
              <div>
                <label className="mb-1.5 block text-xs font-bold text-slate-700" htmlFor="modal-reset-email">
                  Adresse e-mail
                </label>
                <input
                  autoComplete="email"
              ...
              <>
                <div>
                  <label className="mb-1.5 block text-xs font-bold text-slate-700" htmlFor="modal-reset-token">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold text-slate-700" htmlFor="modal-reset-password">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              <label className="mb-1.5 block text-xs font-bold text-slate-700" htmlFor="modal-email">
                Adresse e-mail
              </label>
              <input
                autoComplete="email"
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                <label className="block text-xs font-bold text-slate-700" htmlFor="modal-password">
                  Mot de passe
                </label>
                <button
        ...
        <div className="mt-6 space-y-2 text-center">
          <p className="text-xs text-slate-500">
            ...
            <Link className="font-bold text-[#426b95] hover:text-[#17294b]" href="/inscription" onClick={onClose}>
              Créer un espace
            </Link>
          </p>
        </div>
