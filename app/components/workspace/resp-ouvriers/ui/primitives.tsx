...
/* ------------------------------------------------------------------ */
/* Helper motricité : nombres comptés, halos, grid motif               */
/* ------------------------------------------------------------------ */
...
export function AnimatedNumber({
  ...
  const text = format ? format(count) : Math.round(count).toLocaleString("fr-FR");
  return (
...
export function SectionHeader({
  ...
  return (
    <div className={"flex items-end justify-between gap-4 " + className}>
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#e3a641]/90">{eyebrow}</p>
        <h2 className="mt-1 text-[17px] font-bold tracking-[-0.03em] text-[#e8eefb]">{title}</h2>
      </div>
      {action}
    </div>
  );
...
export function GridBackdrop({ className = "", opacity = 0.5 }: { className?: string; opacity?: number }) {
  return (
...
export function FieldChip({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
...
export function useClientRect<T extends HTMLElement>() {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (!element) return;
      setRect({ width: element.offsetWidth, height: element.offsetHeight });
    };
    measure();
