  ...
  return (
    <section className="relative h-[170vh] bg-[#101c32]" ref={sectionRef}>
      <div className="sticky top-0 h-[100dvh] overflow-hidden">
        ...
        <motion.div className="pointer-events-none absolute inset-x-0 top-[21%] z-20 mx-auto flex max-w-5xl flex-col items-center px-5 text-center text-white sm:top-[24%]" style={{ opacity: prefersReducedMotion ? 1 : titleOpacity, y: prefersReducedMotion ? 0 : contentY }}>
          <div className="pointer-events-auto">{eyebrow}</div>
          <h1 className="mt-5 max-w-5xl text-[clamp(2.6rem,6vw,5.75rem)] font-bold leading-[0.92] tracking-[-0.075em] text-white drop-shadow-[0_5px_24px_rgba(0,0,0,0.38)]">
          ...
          <div className="pointer-events-auto mt-6 max-w-xl text-sm leading-6 text-slate-100 sm:text-base sm:leading-7">{description}</div>
          <div className="pointer-events-auto mt-7">{actions}</div>
          <div className="pointer-events-auto mt-7">{proof}</div>
        </motion.div>

        <motion.div aria-hidden="true" className="absolute inset-x-0 bottom-7 z-20 flex flex-col items-center gap-2 text-center text-[10px] font-bold uppercase tracking-[0.18em] text-white/75" style={{ opacity: prefersReducedMotion ? 0 : titleOpacity }}>
