...
export function CinematicHero({ 
  ...
  // 1. High-Performance Mouse Interaction Logic (gsap.quickTo = zero tween churn)
  useEffect(() => {
    if (!mockupRef.current) return;

    const rotateY = gsap.quickTo(mockupRef.current, "rotationY", {
      duration: 1.2,
      ease: "power3.out",
    });
    const rotateX = gsap.quickTo(mockupRef.current, "rotationX", {
      duration: 1.2,
      ease: "power3.out",
    });
